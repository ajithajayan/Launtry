...
const ProductEditForm = ({ product, fetchProducts, setShowModal }) => {
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setPrice(product.price); // Populate price when editing
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("price", price); // Append price to the form data
    formData.append("brand", selectedBrand?.value);
    formData.append("category", selectedCategory?.value);
    formData.append("unit_type", selectedUnit.value);
    if (image) {
      formData.append("image", image); // Append the image file if it exists
    }

    try {
      await axios.put(`${baseUrl}store/products/${product.id}/`, formData, {
        headers: {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={price}
                  onChange={(e) => setPrice(e.target.value)} // Handle price changes
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                  required
                />
              </div>
